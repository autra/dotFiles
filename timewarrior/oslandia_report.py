    ...
    for session in tracked:
        ...
        if not session.get("tags"):
            ...
            if len(session["tags"]) > 1:
                error_and_exit(
                    "This report can only report one tag per session.")
...
if __name__ == "__main__":
    ...
    nb_half_day = sum(
        [2 if seconds > 60 * 60 * 5 else 1 for seconds in days.values()])
    print("Nb half days       : {}".format(nb_half_day))

    total_hours = sum([seconds / 3600.0 for seconds in totals.values()])
    ...
    tasks = [(seconds / 3600.0, task) for task, seconds in totals.items()]
